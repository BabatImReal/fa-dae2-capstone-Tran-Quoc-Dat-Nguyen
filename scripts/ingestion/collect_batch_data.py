...
DLT_INPUT_DIR = Path(
    config["paths"].get(
        "dlt_input_dir", str(Path(DATA_DIR) / "olistbr_brazilian_ecommerce")
    )
)
...
def main():
    ...
    try:
        ...
        if zip_path.exists():
            ...
            logger.info(
                f"No archive {zip_path} found; checking for files in {DATA_DIR}"
            )
    ...
    csv_files = sorted(dltdir.rglob("*.csv"))
    if csv_files:
