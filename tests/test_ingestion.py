...
def test_fake_data_generation():
    ...
    try:
        ...
        required_fields = [
            'event_id', 'user_id', 'session_id', 'song_id', 'song_title',
            'artist', 'album', 'genre', 'duration_seconds', 'position_seconds',
            'event_action', 'device_type', 'platform', 'timestamp', 'user_premium'
        ]
...
def test_postgresql_ingestion():
    ...
    try:
        from scripts.ingestion.ingest_to_postgre import get_connection, insert_music_transactions
        from scripts.ingestion.collect_fake_data import FakeDataGenerator
        
        # Generate test data
        generator = FakeDataGenerator()
        test_data = generator.add_ingested_at(generator.generate_music_transaction_data(3))
...
def test_end_to_end_pipeline():
    ...
    try:
        ...
        generator = FakeDataGenerator()
        test_data = generator.add_ingested_at(generator.generate_music_transaction_data(5))
